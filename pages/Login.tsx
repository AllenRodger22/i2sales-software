...
const LoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      await login(email, password);                 // autentica e atualiza contexto
      navigate('/dashboard', { replace: true });   // cai no redirector por role
    } catch (err: any) {
      console.error(err);
    } finally {
      setSubmitting(false);
    ...
    <>
      ...
      <form className="mt-8 space-y-4" onSubmit={handleLogin}>
        ...
        <div className="flex items-center justify-end text-sm">
          <button
            type="button"
            onClick={() => { setView('forgot_password'); setLocalError(null); setSuccessMessage(null); }}
      ...
      <p className="mt-6 text-center text-sm text-gray-400">
        Não tem uma conta?{' '}
        <Link to="/register" className="font-medium text-orange-400 hover:text-orange-300">Registre-se</Link>
      </p>
    </>
  );
  ...
  const renderForgotPasswordView = () => (
    <>
      <div>
        <h2 className="text-center text-2xl sm:text-3xl font-light text-white font-poppins">Redefinir Senha</h2>
        <p className="mt-2 text-center text-sm text-gray-300">Digite seu email para receber o link de redefinição.</p>
      </div>
      ...
      <p className="mt-6 text-center text-sm text-gray-400">
        Lembrou a senha?{' '}
        <button
          onClick={() => { setView('login'); setLocalError(null); setSuccessMessage(null); }}
          className="font-medium text-orange-400 hover:text-orange-300"
        >
          Voltar para o Login
        </button>
